...
class Transaksi_model extends CI_Model {
	...
	{
		$data_cart = $this->db->where('barang.kode_barang', $this->input->post('kode_barang'))
	...
	public function ubah_jumlah_cart()
	{
		...
		$stok_awal = $this->get_data_barang_by_id($this->input->post('id_barang'))->stok;
	...
	{
		return $this->db->select('SUM(barang.harga*cart.jumlah) as total')
	...
	public function tambah_transaksi()
	{
		...
		for($i = 0; $i < count($this->get_cart()); $i++)
		{
			...
			$stok_awal = $this->get_data_barang_by_id($this->input->post('id_barang')[$i])->stok;
	...
	{
		return $this->db->select('transaksi.id_transaksi, transaksi.nama_pembeli, transaksi.id_kasir, transaksi.tgl_beli, (SELECT SUM(detail_transaksi.jumlah*barang.harga) FROM detail_transaksi JOIN barang ON barang.id_barang = detail_transaksi.id_barang WHERE id_transaksi = transaksi.id_transaksi ) as total')
	...
	{
		return $this->db->select('barang.nama_barang, kategori.nama_kat, barang.harga, detail_transaksi.jumlah')
